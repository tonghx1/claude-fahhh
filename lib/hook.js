...
function playSound() {
  const soundFile = path.join(__dirname, "fahhhh.wav");
  ...
  if (platform === "darwin") {
    ...
    // Try aplay, then paplay, then pw-play
    execFile("aplay", [soundFile], { timeout: 5000 }, (err) => {
      if (err) {
        execFile("paplay", [soundFile], { timeout: 5000 }, (err2) => {
          if (err2) {
            execFile("pw-play", [soundFile], { timeout: 5000 }, () => {});
          }
        });
      }
    });
  } else if (platform === "win32") {
    // PowerShell one-liner to play WAV
    const cmd = `(New-Object Media.SoundPlayer '${soundFile}').PlaySync()`;
